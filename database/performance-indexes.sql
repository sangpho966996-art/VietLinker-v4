...
CREATE INDEX IF NOT EXISTS idx_marketplace_title_status 
ON marketplace_posts(title, status) WHERE status = 'active';

CREATE INDEX IF NOT EXISTS idx_marketplace_description_status 
...
CREATE INDEX IF NOT EXISTS idx_job_posts_title_status 
...
CREATE INDEX IF NOT EXISTS idx_job_posts_category_status 
ON job_posts(category, status) WHERE status = 'active';

CREATE INDEX IF NOT EXISTS idx_real_estate_title_status 
...
CREATE INDEX IF NOT EXISTS idx_real_estate_location 
ON real_estate_posts(city, state, status) WHERE status = 'active';

CREATE INDEX IF NOT EXISTS idx_business_profiles_search 
ON business_profiles USING gin(to_tsvector('english', business_name || ' ' || COALESCE(description, '')));

CREATE INDEX IF NOT EXISTS idx_business_profiles_type_status 
...
CREATE INDEX IF NOT EXISTS idx_marketplace_created_at 
...
CREATE INDEX IF NOT EXISTS idx_real_estate_created_at 
